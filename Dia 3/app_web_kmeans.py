...
CORES_15 = [
    "#8250df", "#0969da", "#1a7f37", "#cf222e", "#9a6700",
    "#bf3989", "#0550ae", "#2da44e", "#bc4c00", "#6e7781",
    "#6639ba", "#0a7ea4", "#116329", "#a40e26", "#7d4e00",
]
...
def preparar_kmeans_15(lematizado):
    """Gera uma leitura alternativa com k = 15 para comparar com os 4 macrotemas."""
    textos_tokens = lematizado["tokens"].apply(json.loads)
    documentos = [" ".join(tokens) for tokens in textos_tokens]
    ...
    ordem = modelo.cluster_centers_.argsort()[:, ::-1]
    contagens = pd.Series(labels).value_counts().sort_index()
    linhas = []

    for cluster in range(15):
        principais = [termos[i] for i in ordem[cluster, :8]]
        linhas.append({
            "cluster": cluster,
            "quantidade_textos": int(contagens.get(cluster, 0)),
            "rotulo": f"{principais[0]} / {principais[1]}",
            "termos_principais": ", ".join(principais),
        })

    return pd.DataFrame(linhas), projecao_15


dados = carregar_dados()
projecao = preparar_projecao(dados["lematizado"], dados["kmeans"])
topicos_15_df, projecao_15 = preparar_kmeans_15(dados["lematizado"])
...
def fig_distribuicao_k15():
    fig = go.Figure(go.Bar(
        x=[f"C{int(c)}" for c in topicos_15_df["cluster"]],
        y=topicos_15_df["quantidade_textos"],
        marker_color=[CORES_15[int(c)] for c in topicos_15_df["cluster"]],
        text=topicos_15_df["quantidade_textos"],
        textposition="outside",
        hovertext=topicos_15_df["termos_principais"],
        hovertemplate="%{x}<br>%{y} textos<br>%{hovertext}<extra></extra>",
    ))
    fig.update_layout(
        title=dict(text="Distribuição dos textos com k = 15", font=dict(size=14, color=BRANCO)),
        paper_bgcolor=BG,
        plot_bgcolor=CARD,
        font=dict(color=BRANCO),
        xaxis=dict(gridcolor=BORDA, linecolor=BORDA, tickfont=dict(color=CINZA)),
...
def fig_projecao_k15():
    fig = go.Figure()
    for cluster_id in sorted(projecao_15["cluster"].unique()):
        parte = projecao_15[projecao_15["cluster"] == cluster_id]
        termos = topicos_15_df.loc[topicos_15_df["cluster"] == cluster_id, "rotulo"].iloc[0]
        fig.add_trace(go.Scatter(
            x=parte["x"],
            y=parte["y"],
            mode="markers",
            name=f"C{cluster_id} - {termos}",
            marker=dict(
                size=6,
                color=CORES_15[int(cluster_id)],
                opacity=0.7,
    ...
    fig.update_layout(
        title=dict(text="Visualização 2D dos textos com k = 15", font=dict(size=14, color=BRANCO)),
        paper_bgcolor=BG,
        plot_bgcolor=CARD,
        font=dict(color=BRANCO),
        legend=dict(bgcolor=CARD, bordercolor=BORDA, borderwidth=1, font=dict(color=BRANCO, size=10)),
        xaxis=dict(title="Componente SVD 1", gridcolor=BORDA, linecolor=BORDA, tickfont=dict(color=CINZA)),
        yaxis=dict(title="Componente SVD 2", gridcolor=BORDA, linecolor=BORDA, tickfont=dict(color=CINZA)),
        height=560,
        margin=dict(t=55, b=45, l=50, r=20),
...
def tabela_topicos_k15():
    rows = []
    for _, linha in topicos_15_df.iterrows():
        rows.append({
            "Cluster": f"C{int(linha['cluster'])}",
            "Textos": int(linha["quantidade_textos"]),
            "Leitura rápida": linha["rotulo"],
            "Termos principais": linha["termos_principais"],
        })
...
def cards_clusters_k15():
    cards = []
    for _, linha in topicos_15_df.iterrows():
        cluster = int(linha["cluster"])
        termos = [t.strip() for t in linha["termos_principais"].split(",")[:5]]
        cards.append(html.Div(style={
            "backgroundColor": CARD,
            "border": f"2px solid {CORES_15[cluster]}",
            "borderRadius": "8px",
            "padding": "12px",
            "minWidth": "185px",
            "flex": "1",
        }, children=[
            html.Div(f"C{cluster}", style={
                "color": "#ffffff",
                "backgroundColor": CORES_15[cluster],
                "display": "inline-block",
                "padding": "3px 8px",
                "borderRadius": "999px",
                "fontSize": "11px",
            ...
            html.H4(linha["rotulo"], style={"margin": "0 0 6px 0", "fontSize": "13px", "color": BRANCO}),
            html.Div(str(int(linha["quantidade_textos"])), style={
                "color": CORES_15[cluster],
                "fontSize": "24px",
                "fontWeight": "bold",
                "lineHeight": "1.1",
            }),
            html.Div("textos", style={"color": CINZA, "fontSize": "11px", "marginBottom": "8px"}),
            html.Div([html.Span(termo, style={
                "border": f"1px solid {BORDA}",
                "borderRadius": "999px",
                "padding": "3px 7px",
                "margin": "2px",
...
def aba_k15():
    return html.Div(style={"padding": "20px 0"}, children=[
        html.Div(style={
            ...
            "border": f"1px solid {BORDA}",
            "borderLeft": f"5px solid {AMARELO}",
            "borderRadius": "8px",
            "padding": "12px 16px",
            "marginBottom": "16px",
            "color": BRANCO,
            ...
            html.B("E se escolhêssemos k = 15? "),
            "Os grupos ficam mais específicos. Isso ajuda a investigar subtemas, mas a explicação geral fica mais fragmentada do que em k = 4.",
        ]),
        html.Div(style={"display": "flex", "gap": "16px", "flexWrap": "wrap"}, children=[
            dcc.Graph(figure=fig_distribuicao_k15(), style={"flex": "1", "minWidth": "420px"}, config={"displayModeBar": False}),
            dcc.Graph(figure=fig_projecao_k15(), style={"flex": "1.4", "minWidth": "520px"}, config={"displayModeBar": False}),
        ]),
        html.H3("Possíveis subgrupos com k = 15", style={"color": BRANCO, "fontSize": "15px", "margin": "18px 0 10px 0"}),
        html.Div(style={"display": "flex", "gap": "10px", "flexWrap": "wrap", "marginBottom": "18px"}, children=cards_clusters_k15()),
        html.H3("Resumo dos 15 clusters", style={"color": BRANCO, "fontSize": "15px", "marginBottom": "10px"}),
        tabela_topicos_k15(),
    ])
...
app.layout = html.Div(style={
    ...
    dcc.Tabs(
        ...
        children=[
            ...
            dcc.Tab(label="Simulação k = 15", value="k15", style=TAB_STYLE, selected_style=TAB_SEL, children=[aba_k15()]),
        ],
    ),
])
